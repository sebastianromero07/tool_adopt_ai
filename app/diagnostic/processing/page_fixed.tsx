// filepath: c:\Users\aries\tool_adopt_ai\app\diagnostic\processing\page.tsx
'use client';
...
function ProcessingContent() {
  ...
  const sessionId = searchParams.get('sessionId') || localStorage.getItem('lastSessionId') || '';
    ...
    const typeNextCharacter = () => {
      if (lineIndex < terminalLines.length) {
        ...
        if (charIndex < fullText.length) {
          ...
          charIndex++;
          const charDelay = currentLine.type === 'json' ? 5 : 30;
        ...
        timeoutId = setTimeout(() => {
          if (sessionId) {
            ...
            router.push('/diagnostic/roadmap');
          }
        }, 2000);
      }
    };
...
function ProcessingLoadingFallback() {
  return (
    <main className="bg-surface text-on-surface min-h-[calc(100vh-80px)] flex items-center justify-center p-6">
      <div className="text-center">
        <div className="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-[#0077B6]"></div>
        <p className="mt-4 text-[#404850]">Cargando análisis...</p>
      </div>
    </main>
  );
}
